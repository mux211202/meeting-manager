...
export default function AutocompleteEmails({setValue, invitedUsers, hostMail}) {
  ...
  useEffect(() => {
    const getParticipants = async () => {
      ...
      const filteredUsers = queryUser.queryUser.filter((user) => user.email !== hostMail);
  ...
  return (
    <Root>
      <div {...getRootProps()}>
        <Label {...getInputLabelProps()}>Invite Participants</Label>
        <InputWrapper ref={setAnchorEl} className={focused ? "focused" : ""}>
          {value.map((option, index) => (
            <StyledTag label={option} {...getTagProps({ index })} />
          ))}
          <input {...getInputProps()} value={autocompleteValue} onChange={handleInput} />
        </InputWrapper>
      </div>
